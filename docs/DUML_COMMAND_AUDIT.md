...
При исходном аудите байты и порядок профилей не менялись. После восстановления
RC MCU 2026-07-24 из runtime удалены две подтверждённые stick-lock операции
`06:72`, посторонняя запись `max_height=500`, неподтверждённый CE-профиль и
мёртвый legacy keepalive asset. Статический справочник по командам из
бинарников пульта вынесен отдельно:
...
Хэши относятся к исходным файлам до исправления их `description`/`note` и
последующего safety-сокращения. Удалённые профили сохранены здесь как
provenance, а не как текущие runtime assets.
...
## Исторический FCC-профиль и текущий country flow

Таблица ниже фиксирует исходный upstream-профиль из 21 кадра, который
отправлялся в два раунда, как provenance. На проверенных комбинациях дрона и
пульта этот составной профиль приводил к FCC-результату, но это не доказывает
необходимость каждого кадра и универсальность последовательности. В текущем
`fcc.json` из этой таблицы остались только три кадра: `09:27 setForceFcc`
(№ 9), `09:27 ffff0063=3` (№ 10) и `03:F9 c1_regulatory_restriction=0`
(половина `=0` из № 13/14); остальные удалены (см. выше).

С 2026-07-24 country setup отделён от повторяемого core. Изначально целевым
кодом был только `AU`; теперь тот же flow использует сохранённый выбор из
`AU`, `CN`, `US`, `BO`, `RU`, `NL` или `MY`:

1. один read-only `07:19` к `dst=0x09`;
2. при несовпадении `07:30=<selected country>` и проверочный `07:19`;
3. оставшиеся 3 кадра `fcc.json` в два раунда, то есть 6 core writes.

Текущий `fcc.json` имеет SHA-256
`0404608625aa506ab131ff3342dda97c8ccbe48dc86ac41b078c4915295ddd5a`.
С 2026-08-19 core сокращён до трёх кадров с восстановленным смыслом: два
`09:27` SDR-assistant write (`0xffff0048=2` setForceFcc и `0xffff0063=3`) и
одиночный `03:F9 c1_regulatory_restriction=0` (снятие ограничения, без прежнего
импульса `1→0`). Из прежнего 14-кадрового core дополнительно удалены
opaque/no-op кадры `10:58`, `00:00`, `00:32`, `03:AF`, `00:E5`, `06:8C` и
штатные safety-флаги `sdr_lost_prevent_*` (default уже `1`); ещё раньше были
удалены `07:30`-дубликат, `07:18`, слепой `07:19`, два `06:72` и
`max_height=500`.
Один общий кодовый flow используется ручным, Home Point и periodic bootstrap.

С 1.5.48 порядок внутри country flow изменён на read-first: сначала
выполняется read-only `07:19`. Если он вернул выбранный код, `07:30` не
отправляется вообще. При любом другом ответе (включая отсутствие ответа)
выполняется `07:30=<selected country>` и повторный `07:19`; пара повторяется до
трёх раз, пока readback не вернёт выбранный код. Неподтверждённый результат
логируется как `verified=false,
writes=3` и не блокирует остальную последовательность.

Периодический режим с 1.5.48 состоит из одного `07:19` каждые 5 секунд.
Совпадающий регион не порождает записей; расхождение вызывает country write и
повторное применение полного `fcc.json`. Четыре legacy-кадра не отправляются,
а после safety-аудита `fcc_keepalive.json` удалён из runtime assets.

Live `rc331`/WA530 на port `8901`: исходный `07:19` вернул
`00 52 55 00` (`RU`), одна `07:30=AU` получила ACK `00 01`, следующий
`07:19` вернул `00 41 55 00` (`AU`). Два отдельных теста `07:18=RU`
(`sender/cmdType` `2A/40` и точный старый `82/20`) не получили matching ACK;
readback после каждого остался `AU`.

На отдельном `rc520` исходный `07:19` вернул `RU`. Точная старая
`07:18=AU` (`sender=82`, `cmdType=20`, `dst=07`) не получила matching ACK ни
на `8901`, ни на основном FCC-port `40009`; оба readback после тестов остались
`RU`. Таким образом, изменение country state через `07:18` имеет отрицательный
live-результат на проверенных `rc331` и `rc520`, но этот вывод не переносится
на остальные DJI-платформы без отдельного A/B. В частности, для заявленного в
compatibility table RC Plus/RM700 обе операции — действие `07:18` и успешный
`07:30=AU` с последующим `07:19=AU` — остаются `UNTESTED` и требуют проверки
владельцем реального пульта.
...
`07:30`, а `07:19` подтверждает результат перед core.
...
| GPS write | wrapped `03:F9`, `829542c500/01` | 4 цикла по 5 bounded writes (20 всего), затем новый `03:F8` read | `CONFIRMED` для protocol; physical persistence отдельно не доказана |
...
`18:4B/4C`. Полный разбор приёмной стороны (2026-07-25,
...
### `ce_restore.json`: destination не совпадает с получателем `06:72`

Профиль «CE Restore» состоит из одного кадра `06:72` с payload
`00000000000100` — байт в байт первый кадр legacy-keepalive, — но отправляется
не на `dst=0x06`, а на **`dst=0x20`**.

Теперь, когда `06:72` расшифрована как `set stick value lock`
(см. [`RC331_MCU_FIRMWARE.md`](RC331_MCU_FIRMWARE.md)), это выглядит так:

- сама команда к региону/CE отношения не имеет;
- `dst=0x20` в кодировке `(index << 5) | (module_type & 0x1f)` даёт
  `index=1`, `module_type=0`, то есть **не** `rc:0` (`0x06`), где живёт
  единственный известный handler `06:72`;
- в таблице маршрутов RM510 `dji.json` тип `0` не соответствует RC MCU.
...
подтверждается. Профиль и LAN-команда удалены: безопасного или

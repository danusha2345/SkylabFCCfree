...
- после cold boot Auto-FCC сработал: Home Point → 2001 ms settle → pinned
  `40009` → `42/42` → 12 matching ACK → terminal stop; пользователь подтвердил
  физический FCC;
- предыдущий запуск той же связки завершился `monitor_failed` и потребовал
  ручной apply, поэтому воспроизводимость автоматики ещё проверяется.

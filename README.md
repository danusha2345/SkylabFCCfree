...
6. The aircraft-control card is split evenly: GPS on the left and LED on the right. Each side has its own manual refresh and explicit ON/OFF buttons, available without starting Auto FCC first. A manual read or post-write verification makes at most two bounded attempts and stops after the first valid response. GPS readback is confirmed on RC2; GPS writes remain experimental and must be verified in DJI Fly. Neither side polls port `40007` in the background.

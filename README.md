...
6. The aircraft-control card is split evenly: GPS on the left and LED on the right. Each side has its own manual refresh and explicit ON/OFF buttons, available without starting Auto FCC first. A manual read makes at most three bounded attempts. GPS ON/OFF makes at most three `write → delayed single readback` attempts, waiting 1.5 seconds for the asynchronous GNSS state change and one second before a retry; LED ON/OFF makes at most two complete reference-pattern command cycles. Both stop immediately after the requested state is verified. GPS/LED stay on the wrapped `40007` path because live RC Pro 2 tests found no matching readback on `40009` or `8901`. The last validated replies persist across app reopen with a `Last verified` timestamp; a failed refresh does not erase them. Neither side polls port `40007` in the background.
